...
https://github.com/peterbrittain/asciimatics/blob/v1.13/samples/ray_casting.py
"""

import sys
from math import copysign, cos, floor, pi, sin
...
class GameState(object):
    ...
    def safe_update_x(self, new_x: int) -> None:
        """Copied code, dummy docstring"""
        new_x += self.x
    ...
    def safe_update_y(self, new_y: int) -> None:
        """Copied code, dummy docstring"""
        new_y += self.y
    ...
    def safe_update_angle(self, new_angle: int) -> None:
        """Copied code, dummy docstring"""
...
class MiniMap(Effect):
    ...
    def __init__(self, screen: Screen, game_state: GameState, size: int = 5):
    ...
    def _update(self, _: Any) -> None:
        ...
        for mx in range(self._size):
            for my in range(self._size):
                ...
                if (
                    0 <= py < len(self._state.map)
                    and 0 <= px < len(self._state.map[0])
                    and self._state.map[py][px] != " "
                ):
                    colour = Screen.COLOUR_RED
                else:
                    colour = Screen.COLOUR_BLACK
                self._screen.print_at(
                    "  ", self._x + 2 * mx, self._y + my, colour, bg=colour
                )
...
class RayCaster(Effect):
    ...
    def __init__(self, screen: Screen, game_state: GameState):
        super(RayCaster, self).__init__(screen)
        self._state = game_state
        self._block_size = screen.height // 3
        if screen.colours >= 256:
            self._colours = [
                x for x in zip(range(255, 232, -1), [0] * 24, range(255, 232, -1))
            ]
        else:
            self._colours = [(Screen.COLOUR_WHITE, Screen.A_BOLD, 0) for _ in range(6)]
            self._colours.extend(
                [(Screen.COLOUR_WHITE, Screen.A_NORMAL, 0) for _ in range(9)]
            )
            self._colours.extend(
                [(Screen.COLOUR_BLACK, Screen.A_BOLD, 0) for _ in range(9)]
            )
    ...
    def _update(self, _: Any) -> None:
        ...
        for sx in range(0, self._screen.width, 2):
            ...
            side_x = (
                (self._state.x - map_x) if ray_x < 0 else (map_x + 1.0 - self._state.x)
            )
            side_x *= ratio_to_x
            side_y = (
                (self._state.y - map_y) if ray_y < 0 else (map_y + 1.0 - self._state.y)
            )
            side_y *= ratio_to_y

            # Give up if we'll never intersect the map
            while (
                (step_x < 0 and map_x >= 0)
                or (step_x > 0 and map_x < len(self._state.map[0]))
            ) and (
                (step_y < 0 and map_y >= 0)
                or (step_y > 0 and map_y < len(self._state.map))
            ):
                ...
                if 0 <= map_x < len(self._state.map[0]) and 0 <= map_y < len(
                    self._state.map
                ):
            ...
            if hit:
                ...
                colour, attr, bg = self._colours[
                    min(len(self._colours) - 1, int(3 * dist))
                ]
                ...
                for sy in range(wall):
                    self._screen.print_at(
                        text * 2,
                        sx,
                        (self._screen.height - wall) // 2 + sy,
                        colour,
                        attr,
                        bg=0 if self._state.mode == 1 else bg,
                    )
                ...
                if hit_side != last_side:
                    ...
                    for sy in range(wall):
                        self._screen.print_at(
                            "|", sx, (self._screen.height - wall) // 2 + sy, 0, bg=0
                        )
...
class GameController(Scene):
    ...
    def __init__(self, screen: Screen, game_state: GameState):
        ...
        effects = [RayCaster(screen, self._state), self._mini_map]
        super(GameController, self).__init__(effects, -1)

    def process_event(self, event: Event) -> Union[Event, None]:
        """Copied code, dummy docstring."""
        ...
        if isinstance(event, KeyboardEvent):
            ...
            if c in (ord("x"), ord("X")):
                ...
                # Not a recognized key - pass on to other handlers.
                return event
        else:
...
def demo(screen: Screen, game_state: GameState) -> None:
    """Copied code, dummy docstring"""
